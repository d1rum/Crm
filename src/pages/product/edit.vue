...
export default {
  ...
  created(){
    ...
    axios.get('http://172.16.1.66:8001/api/category/')
        .then(({data}) => (this.categories = data))
    axios.get('http://172.16.1.66:8001/api/collection/')
        .then(({data}) => (this.collections = data))
    axios.get('http://172.16.1.66:8001/api/supplier/')
        .then(({data}) => (this.suppliers = data))
    axios.get('http://172.16.1.66:8001/api/unit/')
        .then(({data}) => (this.units = data))
    axios.get('http://172.16.1.66:8001/api/main-warehouse/')
        .then(({data}) => (this.mainWarehouse = data))
    let id = this.$route.params.id
    axios.get('http://172.16.1.66:8001/api/product/'+id)
  ...
  methods:{
    ...
    switchCategorySelect(event){
      ...
      if (id != null) {
        axios.get('http://172.16.1.66:8001/api/select-to-category/'+id)
    ...
    productUpdate(){
      let id = this.$route.params.id
      axios.patch('http://172.16.1.66:8001/api/product/'+id,this.form)
          .then(() => {
