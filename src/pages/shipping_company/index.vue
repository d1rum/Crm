        ...
        <div class="text-center">
          <b-modal id="edit-modal" v-model="displayModal" title="Edit Shipping Company" hide title-class="font-18">
            ...
            <div class="form-group col-md-2 mt-2" v-if="editForm.shipping_company_image != null">
              <img width="100px" height="100px"  :hidden="hideStatus" :src="'http://192.168.43.131:8001'+'/'+editForm.shipping_company_image" id="shipping_company_image">
            </div>
            <br>
                ...
                <div class="table-responsive datatable-vue">
                  ...
                  >
                    ...
                    <template v-slot:cell(shipping_company_image)="{item}">
                      <img id="product_image" :src="'http://192.168.43.131:8001'+'/'+item.shipping_company_image">
                    </template>
...
export default {
  ...
  methods:{
    ...
    addShipCompany(){
      this.$http.post('http://192.168.43.131:8001/api/shipping-company',this.form)
    ...
    getShipComList(){
      this.$http.get('http://192.168.43.131:8001/api/shipping-company')
          .then((res) => {
            this.items = res.data;
            // eslint-disable-next-line no-console
    ...
    editShipComp(id){
      // eslint-disable-next-line no-console
      this.$http.patch('http://192.168.43.131:8001/api/shipping-company/'+id,this.editForm)
          .then((res) => {
            this.items = res.data;
            this.getShipComList();
            this.displayModal = false;
    ...
    deleteShipCom(id){
      ...
      Swal.fire({
        ...
        if (result.value) {
          this.$http.delete('http://192.168.43.131:8001/api/shipping-company/'+id)
              .then(() => {
                this.getShipComList();
