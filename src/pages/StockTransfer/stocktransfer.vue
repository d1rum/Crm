                        ...
                        <div class="col-lg-3 col-md-3 col-sm-6 col-6"  v-for="warehouseItem in filtersearch" :key="warehouseItem.id">
                          <button class="btn btn-sm" @click.prevent="AddToStockTransferCart(warehouseItem.id)">
                            <div class="card" style="width:135px; margin-bottom:5px;">
                              <img :src="'http://192.168.57.114:8001/'+warehouseItem.product_image" id="product_photo" :alt="warehouseItem.product_name" class="card-img-top">
                              <div class="card-body">
                     ...
                     <div class="row">
                       <div class="col-lg-3 col-md-3 col-sm-6 col-6" style="width:135px;" v-for="warehouseItemVariant in filtervariantsearch" :key="warehouseItemVariant.id">
                         <button class="btn btn-sm" @click.prevent="AddToStockTransferCart(warehouseItemVariant.product_id,warehouseItemVariant.vid)">
                           <div class="card" style="width:135px; margin-bottom:5px;">
                             <img :src="'http://192.168.57.114:8001/'+warehouseItemVariant.variant_image" id="variant_photo" :alt="warehouseItemVariant.product_name" class="card-img-top">
                             <div class="card-body">
...
export default {
  ...
  methods:{
    stockTransferInsert(){
      axios.post('http://192.168.57.114:8001/api/store/stock-transfer',this.form)
          .then(() => {
    ...
    allOutOfStock(){
      axios.get('http://192.168.57.114:8001/api/get-out-of-stock')
          .then(({data}) => (this.outOfStocks = data))
          .catch()
    },
    allStockTransferCart(){
      axios.get('http://192.168.57.114:8001/api/stocktransfer/cart/')
    ...
    AddToStockTransferCart(id,variant_id = null){
      ...
      axios.get('http://192.168.57.114:8001/api/add/stocktransfer/cart/'+id+'/'+variant_id+'/'+warehouse_id)
          .then(({data}) => {
    ...
    selectedWarehouseItemVariant(id){
      axios.get('http://192.168.57.114:8001/api/select-from-warehouse-item-variant/'+id)
          .then(({data}) => (this.warehouseItemVariants = data))
          .catch()
    },
    removeItem(id){
      axios.get('http://192.168.57.114:8001/api/remove/stocktransfer/cart/'+id)
          .then(() => {
            // eslint-disable-next-line no-undef
            Reload.$emit('AfterAdd');
    ...
    increment(id){
      let warehouse_id = this.form.from_warehouse_id;
      axios.get('http://192.168.57.114:8001/api/increment/stocktransfer/cart/'+id+'/'+warehouse_id)
          .then(({data}) => {
            if (data === 'unsuccessful') {
              // eslint-disable-next-line no-undef
              Reload.$emit('AfterAdd');
    ...
    decrement(id){
      axios.get('http://192.168.57.114:8001/api/decrement/stocktransfer/cart/'+id)
          .then(() => {
            // eslint-disable-next-line no-undef
            Reload.$emit('AfterAdd');
            // Notification.success()
    ...
    fromWarehouse(){
      axios.get('http://192.168.57.114:8001/api/warehouse')
    ...
    fromWarehouseChange(event){
      ...
      if (id != null) {
        this.show = true;
        axios.get('http://192.168.57.114:8001/api/select-from-warehouse/'+id)
            .then(({data}) => (this.toWarehouses = data))
            .catch()
        axios.get('http://192.168.57.114:8001/api/from/warehouse/change/'+id)
            .then(data => {
              const marker = {
    ...
    toWarehouseChange(event){
      let id = event.target.value;
      if (id != null) {
        axios.get('http://192.168.57.114:8001/api/to/warehouse/change/'+id)
            .then(data => {
              const marker = {
                lat:parseFloat(data.data[0].warehouse_latitude),
                lng:parseFloat(data.data[0].warehouse_longitude),
    ...
    selectedWarehouseItem(id){
      if (id != null) {
        axios.get('http://192.168.57.114:8001/api/select-from-warehouse-item/'+id)
    ...
    allProduct(){
      axios.get('http://192.168.57.114:8001/api/admin/product')
          .then(({data}) => (this.products = data))
          .catch()
    },
    subProduct(id){
      axios.get('http://192.168.57.114:8001/api/getting/product/'+id)
